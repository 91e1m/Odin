...
object BuildHelper : Module(
    ...
    fun renderWorldEvent(event: RenderWorldLastEvent) {
        ...
        if (stunNotification && KuudraUtils.buildDonePercentage > stunNotificationNumber) PlayerUtils.alert("§lGo to stun", playSound = false, color = Color.MINECRAFT_AQUA)
        if (buildHelperDraw)
    ...
    private fun renderUnfinishedWaypoints() {
        KuudraUtils.buildingPiles.forEach {
            Renderer.drawCustomBeacon(it.name, it.positionVector.addVec(0.5), Color.MINECRAFT_DARK_RED, true, increase = false, noFade = !fadeWaypoints, distance = false)
        }
    }
